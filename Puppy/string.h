...
namespace kkli {
	...
	>
		...
		private:
			...
			iterator __capacity;		//[__start,__capacity)：所有字符空间，[__start, __capacity]：所有空间
			Allocator __alloc;
			...
			//释放原有内存
			void __deallocate() { __alloc.deallocate(__start, (__capacity - __start) + 1); } //注意末尾有一个预留空间

			//将[first, last)的共size个字符添加到*this后
			template<typename InputIterator>
			void __append(size_type size, InputIterator first, InputIterator last);

			//将[first,last)所指的size个字符赋值给*this
			template<typename InputIterator>
			void __assign(size_type size, InputIterator first, InputIterator last) {
				__deallocate(); __allocate(size, first, last);
			}
			...
			//将[first1, last1)中size个字符替换为[first2, last2)
			template<typename InputIterator>
		...
		public:
			...
			basic_string(const basic_string& rhs, size_type pos, size_type count = npos, const Allocator& alloc = Allocator());
			basic_string(const_pointer data, size_type count = npos, const Allocator& alloc = Allocator());
			...
			int compare(size_type pos1, size_type count1, const basic_string& rhs, size_type pos2 = 0, size_type count2 = npos)const;
			int compare(const_pointer data)const;
			int compare(size_type pos1, size_type count1, const_pointer data, size_type count2 = npos)const;
			...
			//front / back / begin / end / data / rbegin...
...
namespace kkli {
	...
	typename basic_string<CharType,Traits,Allocator>::size_type
		basic_string<CharType, Traits, Allocator>::get_size(InputIteratorA first, InputIteratorB last) {
		size_type size = 0;
	...
	template<typename CharType, typename Traits, typename Allocator>
	typename basic_string<CharType, Traits, Allocator>::size_type 
		basic_string<CharType, Traits, Allocator>::get_size(const_pointer data) {
		if (data == nullptr) return 0;
		size_type size = 0;
		for (;; ++size) if (data[size] == kkli::char_traits<char>::eof()) break;
		return size;
	}
	...
	typename basic_string<CharType, Traits, Allocator>::size_type basic_string<CharType, Traits, Allocator>::__set_value_by_range(
		...
		size_type index = 0;
		for (; first != last; ++first, ++index) *(addr + index) = *first;
		return index;
	}
	...
	void basic_string<CharType, Traits, Allocator>::__set_value_by_value(iterator addr, size_type count, value_type value) {
		for (size_type i = 0; i < count; ++i) *(addr + i) = value;
	}
	...
	void basic_string<CharType, Traits, Allocator>::__append(size_type size, InputIterator first, InputIterator last) {
		//剩余空间足够
		if (__capacity - __end >= size) __end += __set_value_by_range(__end, first, last);
	...
	void basic_string<CharType, Traits, Allocator>::__replace(const_iterator first1, const_iterator last1, size_type size2,
		...
		//[first1, last1)范围不够放下[first2, last2)
			...
			if (size2 - size1 <= __capacity - __end) {		//无需申请内存
				...
				for (; prev_iter >= last1; --prev_iter, --iter) *iter = *prev_iter;
				auto first = __start + (first1 - __start);
	...
	//operator=(rhs)
	template<typename CharType, typename Traits, typename Allocator>
	basic_string<CharType, Traits, Allocator>& basic_string<CharType, Traits, Allocator>::operator=(const basic_string& rhs) {
	...
	//operator=(data)
	template<typename CharType, typename Traits, typename Allocator>
	basic_string<CharType, Traits, Allocator>& basic_string<CharType, Traits, Allocator>::operator=(const_pointer data) {
	...
	//operator=(init)
	template<typename CharType, typename Traits, typename Allocator>
	basic_string<CharType, Traits, Allocator>& basic_string<CharType, Traits, Allocator>::operator=(std::initializer_list<value_type> init) {
		__deallocate();
		__allocate(init.size(), init.begin(), init.end());
		return *this;
	}
	...
	void basic_string<CharType, Traits, Allocator>::insert(const_iterator pos, InputIterator first, InputIterator last) {
		...
		if (__capacity - __end >= count) {
			...
			for (; dst_iter >= pos; --dst_iter, --src_iter) *dst_iter = *src_iter;
	...
	typename basic_string<CharType, Traits, Allocator>::iterator  basic_string<CharType, Traits, Allocator>::insert(
		...
		//剩余空间足够
		if (__capacity - __end >= count) {
			auto src_iter = __end - 1; //将[pos,__end)所有字符后移count个位置
			auto dst_iter = src_iter + count;
		...
		//剩余空间不足
			...
			size_type capacity = (this->size() + count) * 2;				//预留一半空间
			auto addr = __allocate(capacity);
			size_type index = 0;
	...
	//erase(first, last)
	template<typename CharType, typename Traits, typename Allocator>
	typename basic_string<CharType, Traits, Allocator>::iterator  
		basic_string<CharType, Traits, Allocator>::erase(const_iterator first, const_iterator last) {
	...
	//compare(rhs)
	template<typename CharType, typename Traits, typename Allocator>
	int  basic_string<CharType, Traits, Allocator>::compare(const basic_string& rhs)const {
		return __compare(this->size(), __start, __end, rhs.size(), rhs.cbegin(), rhs.cend());
	}

	//compare(pos1, count1, rhs, pos2, count2)
	template<typename CharType, typename Traits, typename Allocator>
	int  basic_string<CharType, Traits, Allocator>::compare(size_type pos1, size_type count1,
	...
	//compare(data)
	template<typename CharType, typename Traits, typename Allocator>
	int  basic_string<CharType, Traits, Allocator>::compare(const_pointer data)const {
		size_type size = get_size(data);
	...
	void basic_string<CharType, Traits, Allocator>::replace(const_iterator first1, const_iterator last1,
		...
		//[first1, last1)范围不够放下count个字符
			...
			if (count2 - count1 <= __capacity - __end) {		//无需申请内存
				...
				size_type capacity = (this->size() + count2 - count1) * 2;
				auto addr = __allocate(capacity);
				size_type index = __set_value_by_range(addr, __start, first1);		//将前部写入
	...
	void basic_string<CharType, Traits, Allocator>::reverse() {
		...
		for (size_type i = 0; i < mid_size; ++i) 
			kkli::swap(__start[i], __start[size - i - 1]);
	}
}
...
namespace kkli {
	...
	basic_string<CharType, Traits, Allocator> operator+(const basic_string<CharType, Traits, Allocator>& lhs,
		CharType value) {
		kkli::basic_string<CharType, Traits, Allocator> str(lhs.size() + 1);
		str += lhs;
		str.push_back(value);
		return str;
	}
